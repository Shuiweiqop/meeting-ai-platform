...
`processing_stage` is non-null only while `status = 'processing'` — both `completed` and `failed` set it back to `null` (see [pipeline.md](pipeline.md)). Any new code path that sets one without the other produces exactly the stuck-progress-bar failure mode described there; there's no database constraint enforcing the pairing, it's maintained by convention in `ProcessMeetingJob` and the retry controller action alone.
