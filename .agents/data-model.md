...
`segments` (JSON) is the source of truth; `content` (plain text) is a derived join of segments, kept because PDF export, the Slack notification, and (potentially) full-text search read it directly rather than re-deriving from segments. The derivation is `Transcript::contentFromSegments()` — the single definition, called by both `ProcessMeetingJob` (initial write) and `TranscriptController::update` (user edits). Any code that writes `segments` MUST re-derive `content` through it, or PDF/Slack silently show stale text while the on-screen transcript is correct. Don't inline a second join.

## `todo_items.assigned_to` starts as a fuzzy AI guess, correctable by hand
The initial value is resolved via `User::where('name', 'like', '%' . $todo['assignee_name'] . '%')->first()` against Gemini's free-text guess — approximate by design (can be `null`, or a wrong match on overlapping name substrings). The meeting owner can then reassign it via `TodoItemController::update` (assigned_to, manager-only — see http-layer.md), so a wrong AI guess is a fixable starting point, not a stuck state. Code reading `assigned_to` must still handle `null`. Don't "fix" the AI matching into an exact match without confirming — an exact match would silently assign fewer todos when Gemini's name differs slightly (nickname vs. full name).

`due_date` is filled the same way: Gemini resolves spoken deadlines against the meeting date into `YYYY-MM-DD` (`callGeminiForTodos` passes `occurredAt()` as the reference), then `ProcessMeetingJob::parseDueDate` defensively parses it — a null, non-string, or unparseable value degrades to `null`, never throws. It's often `null` (no deadline mentioned), and the owner can set/change it in the todo UI. The calendar renders whatever is set.
